...
public:
	Point2D(const double& inputX,
			const double& inputY); //Creates a new Point2D, with X and Y specified
	Point2D(const int& inputX,
			const int& inputY); //Creates a new Point2D, with X and Y specified
	...
	void random(const int& maxX,
				const int& maxY); //Sets X and Y to a random double, between 0 and the maximum specified
