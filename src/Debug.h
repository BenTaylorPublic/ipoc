...
public:
	...
	static void
	incrementLoopNumber(); //Increments the loop number. Used for logging and writing loop numbers from anwywhere in the program.
	static void
	IPOCLoad(MasterController* input); //Used to received a pointer tot he MasterController. This pointer will then be used to logStatusStrings(), when requested.
